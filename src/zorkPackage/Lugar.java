...
public class Lugar {
	...
	public Item tomarItem(String nombreObjeto) {
		...
		Item obj = this.objetos.stream().filter(x -> x instanceof Item)// Esto pregunta si es un item
				.map(x -> (Item) x)// Esto castea
				.filter(x -> x.getNombre().toLowerCase().equals(nombreObjeto)).findAny().orElse(null);
	...
	public void removerObjeto(Objeto objeto) {
		...
		List<Obstaculo> obstaculos = this.objetos.stream().filter(x -> x instanceof Obstaculo)// Esto pregunta si es un
																								// obstaculo
				.map(x -> (Obstaculo) x)// Esto castea
